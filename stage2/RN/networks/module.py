...
from ..networks.rn import RN_B, RN_L


class G_Net(nn.Module):
    def __init__(self, input_channels, output_channels, residual_blocks, threshold):
        ...
        # Middle
        blocks = []
        for _ in range(residual_blocks):    # residual_blocks=8
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv2d(in_channels=64, out_channels=output_channels, kernel_size=7, padding=0)
        )
    ...
    def forward(self, x, mask):
        # x is [mask_img, predict_gradient]
...
class D_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        conv4 = self.conv4(conv3)   # torch.Size([6, 512, 31, 31])
        conv5 = self.conv5(conv4)   # [12, 1, 30, 30]
...
class AdversarialLoss(nn.Module):
    ...
    def __call__(self, outputs, is_real, is_disc=None):
        if self.type == 'hinge':
            ...
            loss = self.criterion(outputs, labels)  # input, target
            return loss
