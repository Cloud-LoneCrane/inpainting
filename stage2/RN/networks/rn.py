...
class RN_binarylabel(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        input:  x: (B,C,M,N), features
                mask: (B,1,M,N), 1 for foreground regions, 0 for background regions
        output: _x: (B,C,M,N)
        '''
        mask = mask.detach()
    ...
    def rn(self, region, mask):
        ...
        sum = torch.sum(region, dim=[0,2,3])  # (B, C, H, W) -> (C)
        Sr = torch.sum(mask, dim=[0,2,3])    # (B, 1, H, W) -> (1)
        Sr[Sr==0] = 1
        mu = (sum / Sr)     # (B, C) -> (C)
...
class SelfAware_Affine(nn.Module):
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True)    # 所有的C的(h,w)每一个位置求一个均值，构成一个(1, h, w)
        max_out, _ = torch.max(x, dim=1, keepdim=True)  # 最大值(1, h, w)
        x = torch.cat([avg_out, max_out], dim=1)    # x本质上实现注意力机制

        x = self.conv1(x)
        importance_map = self.sigmoid(x)    # (1, h, w)

        gamma = self.gamma_conv(importance_map)     # (1, h, w)
        beta = self.beta_conv(importance_map)       # (1, h, w)
