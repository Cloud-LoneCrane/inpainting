...
from PENNet.core.spectral_norm import use_spectral_norm
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, in_channels=3, out_channels=3, init_weights=True):  # 1046
        super(InpaintGenerator, self).__init__()
        cnum = 32

        self.dw_conv01 = nn.Sequential(
            nn.Conv2d(in_channels, cnum, kernel_size=3, stride=2, padding=1),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(cnum, out_channels, kernel_size=3, stride=1, padding=1),
            nn.Tanh()
        )
    ...
    def forward(self, mask_img, mask):
        # img, mask = input[:, :4], input[:, 4:]
        x = mask_img
        # encoder
        x1 = self.dw_conv01(x)
...
if __name__ == '__main__':
    ...
    tw.draw_model(model, [1, 5, 256, 256])
