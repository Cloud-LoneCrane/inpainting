...
from PENNet.core.dataset import Dataset
from PENNet.core.utils import set_seed, set_device, Progbar, postprocess
from PENNet.core.loss import AdversarialLoss, PerceptualLoss, StyleLoss, VGG19
from PENNet.core import metric as module_metric
...
class Trainer():
    def __init__(self, config, debug=False):
        ...
        net = importlib.import_module('modules.' + config['model_name'])
    ...
    def load(self):
        ...
        if latest_epoch is not None:
            ...
            if self.config['global_rank'] == 0:
                print('Loading modules from {}...'.format(gen_path))
            ...
            if self.config['global_rank'] == 0:
                print('Warnning: There is no trained modules found. An initialized modules will be used.')
    ...
    def save(self, it):
        if self.config['global_rank'] == 0:
            ...
            print('\nsaving modules to {} ...'.format(gen_path))
    ...
    def _train_epoch(self):
        ...
        for images, masks, _ in self.train_loader:
            ...
            feats, pred_img = self.netG(inputs, masks)  # in: [rgb(3) + mask(1)]
            ...
            if feats is not None:
                ...
                for _, f in enumerate(feats):
                    pyramid_loss += self.l1_loss(f, F.interpolate(images, size=f.size()[2:4], mode='bilinear',
                                                                  align_corners=True))
