...
class BASE(pl.LightningModule):
    def __init__(self, fine_tune, ckpt, *args, **kwargs):
        super().__init__()
        ...
        self.eg_model = None
        if fine_tune:
            self.eg_model = vit_seg.load_from_checkpoint(ckpt)
            self.eg_model.setup("predict")
    ...
    def log_mae_mse(self, y, hat, compose, stage="train"):
        mse_hat, mae_hat = self.cal_loss(y, hat)
        mse_com, mae_com = self.cal_loss(y, compose)
        self.log_dict({"{}/mae_hat".format(stage): mae_hat,
                       "{}/mse_hat".format(stage): mse_hat,
                       "{}/mae_com".format(stage): mae_com,
                       "{}/mse_com".format(stage): mse_com})
