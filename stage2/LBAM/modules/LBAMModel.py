...
from LBAM.modules.forwardAttentionLayer import ForwardAttention
from LBAM.modules.reverseAttentionLayer import ReverseAttention, ReverseMaskConv
from LBAM.modules.weightInitial import weights_init
...
class VGG16FeatureExtractor(nn.Module):
    def __init__(self):
        ...
        vgg16.load_state_dict(torch.load('LBAM/vgg16-397923af.pth'))
...
class LBAMModel(nn.Module):
    def __init__(self, inputChannels, outputChannels):
        ...
        # reverse mask conv
        self.reverseConv1 = ReverseMaskConv(1, 64)
    ...
    # def forward_(self, inputImgs, masks):
    #     edge = inputImgs[:, 3:4, ...]
    #
    #     ef1, mu1, skipConnect1, forwardMap1 = self.ec1(inputImgs, masks)
    #     ef2, mu2, skipConnect2, forwardMap2 = self.ec2(ef1, mu1)
    ...
    #     dcFeatures6 = self.dc6(skipConnect1, dcFeatures5, concatMap1)
    #
    #     edge = edge.resize_((dcFeatures6.shape[0], 1, dcFeatures6.shape[-1], dcFeatures6.shape[-2]))
    #     feature_edge = torch.cat((dcFeatures6, edge), 1)
    #     # dcFeatures7 = self.dc7(dcFeatures6)
    #     dcFeatures7 = self.dc7(feature_edge)
    #
    #     output = (self.tanh(dcFeatures7) + 1) / 2
    #
    #     return output