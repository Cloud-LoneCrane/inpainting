...
from LBAM.loss.metric_tensor import PSNR, ssim, MAE, MSE

# modified from WGAN-GP
def calc_gradient_penalty(netD, real_data, fake_data, masks, Lambda):
...
class InpaintingLossWithGAN(nn.Module):
    ...
    def forward(self, input, mask, output, gt, count, epoch, image_freq):
        ...
        if count % image_freq == 0:
            self.writer.add_image('GroundTruth', gt[0], count)
            self.writer.add_image('result', output_comp[0], count)

        holeLoss = 6 * self.l1((1 - mask) * output, (1 - mask) * gt)
        validAreaLoss = self.l1(mask * output, mask * gt)

        # add by users
        output_psnr = PSNR(torch.max(gt))(gt, output_comp)
        output_ssim = ssim(gt, output_comp)
        output_mse = MSE(gt, output_comp)
        output_mae = MAE(gt, output_comp)
        ...
        self.writer.add_scalar('Train_eva/PSNR', output_psnr.item(), count)
        self.writer.add_scalar('Train_eva/SSIM', output_ssim.item(), count)
        self.writer.add_scalar('Train_eva/MSE', output_mse.item(), count)
        self.writer.add_scalar('Train_eva/MAE', output_mae.item(), count)

        GLoss = holeLoss + validAreaLoss + prcLoss + styleLoss + 0.1 * D_fake
        self.writer.add_scalar('Generator/Joint loss', GLoss.item(), count)    
        return GLoss.sum()

    def test_log(self, gt, output, t_psnr, t_ssim, t_mse, t_mae, epoch):
        self.writer.add_scalar('Test_eva/PSNR', t_psnr, epoch)
        self.writer.add_scalar('Test_eva/SSIM', t_ssim, epoch)
        self.writer.add_scalar('Test_eva/MSE', t_mse, epoch)
        self.writer.add_scalar('Test_eva/MAE', t_mae, epoch)
        self.writer.add_image('Test_GroundTruth', gt[0], epoch)
        self.writer.add_image('Test_result', output[0], epoch)
