...
from GMCNN.modules.basemodel import BaseModel
from GMCNN.modules.basenet import BaseNet
...
from GMCNN.util.utils import generate_mask
...
class InpaintingModel_GMCNN(BaseModel):
    def __init__(self, in_channels, act=F.elu, norm=None, opt=None):
        ...
        if self.opt.pretrain_network is False:
            ...
            self.optimizer_D = torch.optim.Adam(filter(lambda x: x.requires_grad, self.netD.parameters()),
                                                lr=opt.lr,
                                                betas=(0.5, 0.9))
