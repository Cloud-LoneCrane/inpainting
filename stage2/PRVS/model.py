...
from PRVS.utils.io import load_ckpt
from PRVS.utils.io import save_ckpt
...
class PRVSModel():
    ...
    def initialize_model(self, path_g=None, path_d=None, train=True):
        ...
        try:
            ...
            self.iter = 0
            print('No trained modules, train from beginning')
    ...
    def train(self, train_loader, save_path, finetune=False, writer=None):
        ...
        while keep_training:
            for items in train_loader:
                ...
                self.iter += 1
                if self.iter % 50 == 0:
                    writer.add_images(tag="train/inputs", img_tensor=masked_images, global_step=step)
                    writer.add_images(tag="train/inputs_gradient", img_tensor=masked_edges, global_step=step)
                    writer.add_images(tag="train/inputs_masks", img_tensor=masks, global_step=step)
                    writer.add_images(tag="train/gt_images", img_tensor=gt_images, global_step=step)
                    writer.add_images(tag="train/predict_merged", img_tensor=self.comp_B, global_step=step)

                    writer.add_scalar(tag="train/l1_loss", scalar_value=self.l1_loss_val/50, global_step=step)
    ...
    def test(self, test_loader, result_save_path):
        img_dir = os.path.join(result_save_path, "img")
        result_dir = os.path.join(result_save_path, "result")
        ...
        for items in test_loader:
            ...
            for k in range(comp_B.size(0)):
                ...
                file_path = '{:s}/img_{:d}.png'.format(img_dir, count)
                save_image(grid, file_path)
                
                grid = make_grid(comp_B[k:k+1])
                file_path = '{:s}/img_{:d}.png'.format(result_dir, count)
                save_image(grid, file_path)
